...
export default function UpdateComponentModal({
  ...
  const columnDefs: ColDef[] = [
    { field: "id", hide: true },
    {
      headerName: t("updateModal.componentColumnHeader"),
      field: "display_name",
  ...
  return (
    ...
    >
      ...
      <BaseModal.Header>
        <span className="">
          {isMultiple
            ? t("updateModal.titleMultiple")
            : t("updateModal.titleSingle", {
      ...
      <BaseModal.Content overflowHidden>
        <div className="flex flex-col gap-6">
          <div className="flex flex-col gap-3 text-sm text-muted-foreground">
            {isMultiple ? (
              <p>{t("updateModal.breakingChangesWarning")}</p>
            ) : (
              <>
                <p>{t("updateModal.singleUpdateWarning")}</p>
                <p>{t("updateModal.sidebarLatestVersion")}</p>
              </>
            )}
          </div>
          {isMultiple && (
      ...
      <BaseModal.Footer
        submit={{
          label:
            components.length > 1
              ? t("updateModal.updateButton_plural")
